...
def authentification():
    if request.method == 'POST':
        ...
        if request.form['username'] == 'jeanmoul2' and request.form['password'] == 'aze!123': # password à cacher par la suite
...
def supprimer_livres():
    ...
    # Exécution de la requête SQL pour insérer un nouveau client
    cursor.execute('DELETE FROM livres WHERE id=?', (titre,))
    conn.commit()
    conn.close()
    return redirect('/livres/')  # Rediriger vers la page d'accueil après l'enregistrement
...
def rechercher_livres():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT * FROM livres WHERE titre LIKE ? OR auteur LIKE ?", ('%' + terme_recherche + '%', '%' + terme_recherche + '%'))
